...
<div id="webDesignProject">
    <h2 class="title-h2">Jacadi Model</h2>
    <div class="webDesignProject-all-block wrapper">

        <?php

        $req = $bdd->query("SELECT * FROM `projects` WHERE name LIKE 'J%'");
        ...
        <?php
        $req = $bdd->query("SELECT * FROM `jacadi`");
        ...
        while ($don = $req->fetch()) {
            ...
            echo '<img class="img-responsive project-image" src="assets/jacadi/' . $don['page1'] . '" alt="' . $don['name'] . '">';
            echo '</div>';
